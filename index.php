...
foreach($pipesArray as $pipeData){  //see 3-pipeDataSummary for $pipesArray
	...
	}else{$sectionMainContent->addTitle('Pipe No. '.$pipeData->pipeNo, 1);}
	$tablePipeCalculations = $sectionMainContent->addTable('tsPlain');
	$tablePipeCalculations->addRow($rsHeight);  //I guess I can't configure a row.
	$tablePipeCalculations->addCell(Converter::inchtotwip(2.62),$csVbottomGrey)->addText("Definitions", 'fsNormal', 'psCenterTable3');
